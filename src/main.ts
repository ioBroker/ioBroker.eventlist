...
/** A journal query that may reach into the monthly files */
interface ArchiveFilter extends JournalFilter {
    /** Read the archived months as well, not only the ring buffer */
    archive?: boolean;
    /** Only these cycles, in the order the query gives them: what a table shows, exported */
    ids?: string[];
}
...
export class EventList extends Adapter {
    ...
    async #onReady(): Promise<void> {
        ...
        this.#alarmClasses = buildAlarmClasses(this.config.alarmClasses);
        this.#journalLength = parseInt(this.config.journalLength as string, 10);
        this.#journalLength = isNaN(this.#journalLength) ? 1000 : this.#journalLength;
        this.#journalArchive =
            this.config.journalArchive === true || (this.config.journalArchive as unknown as string) === 'true';
    ...
    async #onStateChange(id: string, state: ioBroker.State | null | undefined): Promise<void> {
        if (id === `${this.namespace}.triggerPDF` && state && !state.ack && state.val) {
            ...
            if (this.#states[id].messagesOnly && hasMessage(this.#states[id])) {
            ...
            if (obj.command === 'insert') {
                ...
                try {
                    ...
                    if (request.state === 'gone') {
                        ...
                        const cls = resolveAlarmClass(request.alarmClass, this.#alarmClasses);
                        await this.#applyMessageChange(
                            this.#raise(
                                this.#messages,
                                {
                ...
                if (obj.callback) {
                    const filter = (obj.message || {}) as unknown as ArchiveFilter;
                    const cycles = queryJournal(await this.#journalOf(filter), filter);
                ...
                if (obj.callback) {
                    const filter = (obj.message || {}) as unknown as ArchiveFilter;
                    let cycles = queryJournal(await this.#journalOf(filter), filter);
                    ...
                    this.sendTo(
                        ...
                        obj.command,
                        {
                            csv: toCsv(cycles, this.#csvTexts()),
    ...
    async #onObjectChange(id: string, obj: ioBroker.Object | null | undefined): Promise<void> {
        const { changed, messagesChanged } = await this.#updateStateSettings(
            id,
            obj as ioBroker.StateObject | null | undefined,
        );
    ...
    async #resyncMessages(id: string): Promise<void> {
        ...
        for (const message of this.#messages.filter(item => item.stateId === id && !configured.has(item.id))) {
            ...
            const next = clearMessage(list, message.id, now, this.#flapping);
            list = next.list;
    ...
    async #loadMessages(): Promise<void> {
        ...
        this.#journal = limitJournal(
            parseJournal(journal?.val, text => this.log.warn(text)),
            this.#journalLength,
        );
    ...
    #activeMessageSeverity(id: string): number | undefined {
        const message = this.#messages.find(item => item.id === id && item.active);
        if (!message) {
            return undefined;
        }
        return message.severity ?? DEFAULT_SEVERITY[message.level].normal;
    }
    ...
    async #applyMessageChange(change: MessageChange): Promise<void> {
        ...
        for (const { transition, message } of change.transitions) {
            ...
            journalChanged = true;
            const openBefore = findOpenCycle(this.#journal, message.id);
            this.#journal = applyToJournal(
                this.#journal,
                transition,
                message,
                now,
                change.list.some(item => item.id === message.id),
            );
     ...
     */
    async #journalOf(filter: ArchiveFilter): Promise<AlarmCycle[]> {
        ...
        return mergeCycles(await this.#readArchive(filter.from, filter.to), this.#journal);
    }
    ...
    async #readArchiveFile(month: string): Promise<string> {
        try {
            const result: unknown = await this.readFileAsync(this.name, archiveFileName(month));
            const data =
                result && typeof result === 'object' && 'file' in result
                    ? (result as { file: string | Buffer }).file
     ...
     */
    async #readArchive(from?: number, to?: number): Promise<AlarmCycle[]> {
        const wanted = from === undefined ? null : new Set(monthsBetween(from, to ?? Date.now()));
        ...
        const lists: AlarmCycle[][] = [];
        for (const entry of files) {
            const month = entry.file.replace(/\.jsonl$/, '');
            if (entry.isDir || !/^\d{4}-\d{2}$/.test(month) || (wanted && !wanted.has(month))) {
                continue;
            }
            lists.push(parseJsonLines(await this.#readArchiveFile(month)));
        }

        return mergeCycles(...lists);
    }
     ...
     */
    #queueForArchive(cycle: AlarmCycle): void {
    ...
    async #flushArchive(): Promise<void> {
        ...
        const byMonth = new Map<string, AlarmCycle[]>();
        for (const cycle of queue) {
            const month = monthKey(cycle.activatedAt);
        ...
        for (const [month, cycles] of byMonth) {
            try {
                ...
                const merged = mergeCycles(parseJsonLines(await this.#readArchiveFile(month)), cycles);
                await this.writeFileAsync(this.name, archiveFileName(month), toJsonLines(merged));
                this.log.debug(`${cycles.length} alarm cycle(s) archived in ${archiveFileName(month)}`);
            } catch (e) {
    ...
    async #publishJournal(): Promise<void> {
        this.#journal = limitJournal(this.#journal, this.#journalLength);
    ...
    async #updateStateSettings(
        id: string,
        obj: ioBroker.StateObject | null | undefined,
    ): Promise<{ changed: boolean; messagesChanged: boolean }> {
