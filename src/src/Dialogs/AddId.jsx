...
import { I18n, Confirm as ConfirmDialog, SelectID } from '@iobroker/adapter-react-v5';
...
class AddIdDialog extends Component {
    ...
    writeSettings(ids, cb) {
        if (!ids?.length) {
            ...
            this.props.socket.getObject(id).then(obj => {
                if (obj?.common) {
                    obj.common.custom ||= {};
                    const newSettings = EditState.getSettings(
    ...
    removeSettings(cb) {
        this.props.socket.getObject(this.state.ids[0]).then(obj => {
            if (obj?.common?.custom?.[this.namespace]) {
                obj.common.custom[this.namespace] = null;
                this.props.socket.setObject(this.state.ids[0], obj).then(() => cb?.());
            } else {
                cb?.();
            }
        });
        ...
        return (
            ...
            >
                ...
                <DialogContent>
                    ...
                    {tabs}
                    {this.state.settings[this.state.currentId] ? (
                        <EditStateComponent
                ...
                <DialogActions>
                    ...
                    <Button
                        ...
                        variant="contained"
                        onClick={() => this.writeSettings([...this.state.ids], () => this.props.onClose())}
                        startIcon={<SaveIcon />}
                        color="primary"
                    >
