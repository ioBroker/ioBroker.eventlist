...
class Options extends Component<OptionsProps, OptionsState> {
    ...
    constructor(props: OptionsProps) {
        ...
        let expanded: string[];
        try {
            expanded = JSON.parse(window.localStorage.getItem('eventlist.options.expanded') || '[]');
        } catch {
            expanded = [];
        }
    ...
    componentDidMount(): void {
        void this.props.socket.subscribeState(this.aliveId, this.onAliveChanged);
    }
    ...
    /** Modify a copy of `defaultStringTexts` and store it */
    updateStringTexts(modify: (texts: DefaultStringText[]) => void): void {
        const defaultStringTexts: DefaultStringText[] = this.props.native.defaultStringTexts
            ? JSON.parse(JSON.stringify(this.props.native.defaultStringTexts))
            : [];
        modify(defaultStringTexts);
        this.props.onChange('defaultStringTexts', defaultStringTexts);
    }

    renderStringTexts(): JSX.Element {
        return (
            <Accordion
                    ...
                    {this.state.expanded.includes('state_string') ? (
                        <Fab
                            ...
                            onClick={e => {
                                e.preventDefault();
                                e.stopPropagation();
                                this.updateStringTexts(texts => texts.push({ value: '', text: '', color: '#000000' }));
                            }}
                        >
                            <IconAdd />
                        </Fab>
                        ...
                        ? this.props.native.defaultStringTexts.map((item, i) => (
                              ...
                              >
                                  <TextField
                                      ...
                                      value={item.value}
                                      type="text"
                                      onChange={e => this.updateStringTexts(texts => (texts[i].value = e.target.value))}
                                      margin="normal"
                                  />
                                  <TextField
                                      variant="standard"
                                      ...
                                      value={item.text}
                                      type="text"
                                      onChange={e => this.updateStringTexts(texts => (texts[i].text = e.target.value))}
                                      margin="normal"
                                  />
                                  <ColorPicker
                                      value={item.color}
                                      ...
                                      onChange={color => this.updateStringTexts(texts => (texts[i].color = color))}
                                  />
                                  <IconButton
                                      style={styles.buttonDelete}
                                      title={I18n.t('Delete value')}
                                      color="secondary"
                                      onClick={() => this.updateStringTexts(texts => texts.splice(i, 1))}
                                  >
                                      <IconDelete />
                                  </IconButton>
                              </div>
        ...
        return (
            <form style={styles.tab}>
                <Logo
                    ...
                    onError={text => this.props.onError?.(text)}
...
export default withWidth()(Options) as unknown as ComponentType<Omit<OptionsProps, 'width'>>;
